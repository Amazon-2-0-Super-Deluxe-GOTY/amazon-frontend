                                ...
                                {form.getValues("country").length !== 0 ? (
                                  stateQuery?.isLoading ? (
                                    ...
                                    stateQuery?.data?.map((state) => (
                                      <SelectItem key={state.id} value={state.iso2}>{state.name}</SelectItem>
                                    ))
                                  )
                                ) : (
                                  <SelectGroup>
                                    <SelectLabel>First you need to select a country.</SelectLabel>
                                  </SelectGroup>
                                )}
                              </SelectContent>
                            </Select>
                        ...
                        <div>
                          ...
                          <FormControl>
                            <Select value={field.value} onValueChange={field.onChange}>
                              ...
                              <SelectContent>
                                {selectCountry.length !== 0 && selectState.length !== 0 ? (
                                  cityQuery?.isLoading ? (
                                    <SelectGroup>
                                      <SelectLabel>Loading...</SelectLabel>
                                    </SelectGroup>
                                    ...
                                    cityQuery?.data?.length === 0 ? (
                                      <SelectGroup>
                                        <SelectLabel>There are no cities there.</SelectLabel>
                                      </SelectGroup>
                                    ) : ( 
                                  ...
                                  <SelectGroup>
                                    <SelectLabel>First you need to select country and state.</SelectLabel>
                                  </SelectGroup>
                                )}
                              </SelectContent>
                            </Select>
        ...
        <section className="w-full">
          <div className="w-full flex flex-col gap-4 rounded-lg bg-gray-100 p-6">
            ...
            <div className="w-full flex justify-end items-center gap-6">
              ...
              <div>
                <span className="text-xl md:text-3xl font-medium text-black">${headerData.whole}</span>
                <sup className="text-xs md:text-lg font-medium text-black">{headerData.fraction}</sup>
              </div>
            </div>
...
function GetStateData(stateIso2:string) {
  const stateQuery = useQuery<IDeliveryData[]>({ 
    ...
    queryFn: stateIso2 ? async () => { 
      return await fetch(`/api/states?ciso=${stateIso2}`).then((res) => res.json());
    } : skipToken,
    refetchOnWindowFocus: false,
  });
...
function GetCityData(countryIso2:string, stateIso2:string) {
  const cityQuery = useQuery<IDeliveryData[]>({ 
    ...
    queryFn: countryIso2 && stateIso2 ? async () => { 
      return await fetch(`/api/cities?ciso=${countryIso2}&siso=${stateIso2}`).then((res) => res.json());
    } : skipToken,
    refetchOnWindowFocus: false,
  });
