...
export const MyOrderCard = ({
  ...
  const additionalInfo = {
    ...
    adress:
      order.deliveryAddresses[0].country +
      ", " +
      order.deliveryAddresses[0].state +
      ", " +
      (order.deliveryAddresses[0].city
        ? order.deliveryAddresses[0].city +
          ", " +
          order.deliveryAddresses[0].postIndex
        : order.deliveryAddresses[0].postIndex),
    paymentType: order.paymentMethod,
    dateDelivered: dateOfArrived,
  };
      ...
      <CardContent className="w-full p-0">
        <div className="flex max-lg:flex-col w-full lg:justify-between lg:items-center py-5 px-4 max-lg:gap-4">
          <div className="flex flex-col justify-center items-start gap-1 lg:gap-2">
            ...
            <span className="text-sm md:text-base text-halftone font-medium">
              Arrived on {dateOfArrived}
            </span>
          </div>
